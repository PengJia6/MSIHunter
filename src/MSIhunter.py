...
def argumentProcress():
    ...
    parser.add_argument('-m','--Microsatellite_configure',required=True,type=str,nargs=1,default=["NA"],help="path of the Microsatellite configure files [required]")
...
def loadMicroSatellite():
    ...
    for cancer,info in pdMSConfigure.iterrows():
        if not  os.path.isfile(info["Discriminative_Microsatellites_path"]):
            print("[ERROR] This file "+info["Discriminative_Microsatellites_path"]+"is not exist! Please check it again!")
            return False
        dfMicroSatellites[cancer]=pd.read_csv(info["Discriminative_Microsatellites_path"],index_col=0)
        print("[INFO] Loading Microsatellite file for "+cancer+" from " + info["Discriminative_Microsatellites_path"] + " ...")
        # dfMicroSatellites = pd.read_csv(arguments["Microsatellite"],index_col=0)
...
def procressOneBam(caseid):
    ...
    try:
        ...
        fileRes.write(
            ...
            "NumberOfInstableMicrosatellites: "+str(InstableNum)+"\n"
...
def resultOut():
    ...
    try:
        for caseid in caseInfo.index:
            caseInfo.loc[caseid,"MSIScore(%)"]=open(arguments["workspace"]+"detailInfo/"+str(caseid)+"/"+str(caseid)).readlines()[-1].split(": ")[-1][:-2]
